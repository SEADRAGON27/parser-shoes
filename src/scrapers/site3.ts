...
class MdFashion implements IScraperInterface {
    ...
    async parse(userData: userDTO) {
        ...
        await page.goto('https://md-fashion.ua', { waitUntil: 'load' });
        ...
        await wait(3000);
        const filter = await page.$('.mobile-filters div:nth-child(1)');
        if (filter === null) {
            await browser.close();
            return;
        }
        ...
        while (true) {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                itemsLinks = { link: link, price: priceModel,image:imageElement };
