...
class NewBalance implements IScraperInterface {
    ...
    async parse(userData: userDTO) {
        ...
        const filter = await page.$('.icon-filters');
        if (filter === null) {
            await browser.close();
            return;
        }
        ...
        while (true) {
            ...
            const images= await page.$$('.product-item__image picture source:nth-child(1)');
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                const text = await element.evaluate(
                    (el: Element): string | undefined => el.textContent?.toLowerCase(),
                );
                ...
                if (text?.includes(userData.model.toLowerCase())) {
                    ...
                    itemsLinks = { link: link, price: priceModel,image:imagE};
